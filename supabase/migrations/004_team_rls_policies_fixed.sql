-- Row-Level Security Policies for Multi-Tenant Team System (FIXED VERSION)
-- This migration implements strict data isolation between teams

-- ============================================================================
-- 1. DROP EXISTING POLICIES (COMPREHENSIVE)
-- ============================================================================

-- Drop all existing policies on all tables to ensure clean slate
...
-- Drop policies on new tables (may not exist yet, but that's ok)
...
DROP POLICY IF EXISTS "Team admins can view domain mappings" ON public.team_domain_mapping;
DROP POLICY IF EXISTS "Team admins can create domain mappings" ON public.team_domain_mapping;
DROP POLICY IF EXISTS "Team admins can update domain mappings" ON public.team_domain_mapping;
DROP POLICY IF EXISTS "Team admins can delete domain mappings" ON public.team_domain_mapping;
DROP POLICY IF EXISTS "Team members can view dashboard widgets" ON public.dashboard_widgets;
DROP POLICY IF EXISTS "Team admins can create dashboard widgets" ON public.dashboard_widgets;
DROP POLICY IF EXISTS "Team admins can update dashboard widgets" ON public.dashboard_widgets;
DROP POLICY IF EXISTS "Team admins can delete dashboard widgets" ON public.dashboard_widgets;
...
CREATE POLICY "Team admins can add members" ON public.team_members
  ...
  WITH CHECK (
    EXISTS (
      ...
      AND tm_check.role = 'admin'
    )
    OR 
    -- Allow the trigger to add the team creator as admin
...
-- ============================================================================
-- 9. CREATE/REPLACE TRIGGER TO AUTO-ADD TEAM CREATOR AS ADMIN
-- ============================================================================
...
-- ============================================================================
-- 10. CREATE/REPLACE ADDITIONAL SECURITY FUNCTIONS
-- ============================================================================
